...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'AEStudent'
        db.create_table('teleforma_ae_student', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('user', self.gf('telemeta.models.core.ForeignKey')(related_name='ae_student', unique=True, to=orm['auth.User'])),
            ('platform_only', self.gf('telemeta.models.core.BooleanField')(default=False)),
        ))
        db.send_create_signal('teleforma', ['AEStudent'])

        # Adding M2M table for field period on 'AEStudent'
        db.create_table('teleforma_ae_student_period', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('aestudent', models.ForeignKey(orm['teleforma.aestudent'], null=False)),
            ('period', models.ForeignKey(orm['teleforma.period'], null=False))
        ))
        db.create_unique('teleforma_ae_student_period', ['aestudent_id', 'period_id'])

        # Adding M2M table for field courses on 'AEStudent'
        db.create_table('teleforma_ae_student_courses', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('aestudent', models.ForeignKey(orm['teleforma.aestudent'], null=False)),
            ('course', models.ForeignKey(orm['teleforma.course'], null=False))
        ))
        db.create_unique('teleforma_ae_student_courses', ['aestudent_id', 'course_id'])

    def backwards(self, orm):
        # Deleting model 'AEStudent'
        db.delete_table('teleforma_ae_student')

        # Removing M2M table for field period on 'AEStudent'
        db.delete_table('teleforma_ae_student_period')

        # Removing M2M table for field courses on 'AEStudent'
        db.delete_table('teleforma_ae_student_courses')
