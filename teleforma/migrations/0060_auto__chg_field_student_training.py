...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing M2M table for field period on 'Student'
        db.delete_table('teleforma_student_period')

        # Adding M2M table for field trainings on 'Student'
        db.create_table('teleforma_student_trainings', (
        ...
        db.create_unique('teleforma_student_trainings', ['student_id', 'training_id'])
        ...
        if not db.dry_run:
            ...
            for student in orm.Student.objects.all():
                training = orm.Training.objects.get(id=student.training.id)
                student.trainings.add(training)
                student.save()

    def backwards(self, orm):
        # Adding M2M table for field period on 'Student'
        db.create_table('teleforma_student_period', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('student', models.ForeignKey(orm['teleforma.student'], null=False)),
            ('period', models.ForeignKey(orm['teleforma.period'], null=False))
        ))
        db.create_unique('teleforma_student_period', ['student_id', 'period_id'])

        # Removing M2M table for field trainings on 'Student'
        db.delete_table('teleforma_student_trainings')


        # Changing field 'Student.training'
        db.alter_column('teleforma_student', 'training_id', self.gf('telemeta.models.core.ForeignKey')(default=1, to=orm['teleforma.Training']))
    ...
    models = {
        ...
        'notes.note': {
            ...
            'date': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2013, 7, 4, 0, 0)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'teleforma.student': {
            ...
            'training': ('telemeta.models.core.ForeignKey', [], {'related_name': "'student'", 'on_delete': 'models.SET_NULL', 'default': 'None', 'to': "orm['teleforma.Training']", 'blank': 'True', 'null': 'True'}),
            'trainings': ('django.db.models.fields.related.ManyToManyField', [], {'blank': 'True', 'related_name': "'student_profile'", 'null': 'True', 'symmetrical': 'False', 'to': "orm['teleforma.Training']"}),
            'user': ('telemeta.models.core.ForeignKey', [], {'related_name': "'crfpa_student'", 'unique': 'True', 'to': "orm['auth.User']"}),
